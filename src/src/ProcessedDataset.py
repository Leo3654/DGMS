...
class ProcessedDataset(object):
    def __init__(self, name, root, log_path, skip_check, train_sample_size, random_split):
        ...
        self.graph_id_list = []
        self.graph_id_list = list(range(0, 497))
        for j in range(500, 998):
            self.graph_id_list.append(j)
        for i in range(1000, 20603):
            if os.path.exists("../../../cosqa/VarCosqa/code_processed/code_"+str(i)+".pt"):
        ...
        if os.path.exists(os.path.join(root, 'split.json')): #Henry: this file is only available at Baidu
            ...
            if random_split:
                ...
                self.split_ids['test'] = random.sample(all_ids, 497)
                all_ids = [i for i in all_ids if i not in self.split_ids['test']]
                self.split_ids['valid'] = random.sample(all_ids, 498)
